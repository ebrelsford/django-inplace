...
class Place(models.Model):
    ...
    def get_absolute_url(self):
        return ('inplace:%s_%s_detail' % self._app_and_object(), (), {
            'pk': self.pk,
        })

    @models.permalink
    def get_geojson_url(self):
        return ('inplace:%s_%s_detail_geojson' % self._app_and_object(), (), {
            'pk': self.pk,
        })

    @models.permalink
    def get_popup_url(self):
        return ('inplace:%s_%s_detail_popup' % self._app_and_object(), (), {
            'pk': self.pk,
        })
    ...
    def get_list_geojson_url(cls):
        ...
        return reverse('inplace:%s_%s_geojson' % cls._app_and_object())
