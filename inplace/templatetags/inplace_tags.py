...
@register.tag
def inplace_setting(parser, token):
    """
    Get an inplace setting.

    {% inplace_setting <setting_name> %}
    """
    setting_name = None
    ...
    if len(params) == 2:
        ...
        raise template.TemplateSyntaxError(
            'inplace_setting tag has the following syntax: '
            '{% inplace_setting <setting_name>')
    return InplaceSettingNode(setting_name)


class InplaceSettingNode(template.Node):
